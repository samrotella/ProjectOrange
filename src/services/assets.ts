...
function fromRow(row: Record<string, unknown>): Asset {
  return {
    ...
    priority: (row.priority as Asset['priority']) ?? undefined,
    quantity: (row.quantity as number) ?? undefined,
    installYear: (row.install_year as number) ?? undefined,
    expectedLifespan: (row.expected_lifespan as number) ?? undefined,
...
function toRow(a: Partial<Omit<Asset, 'id' | 'createdAt' | 'updatedAt'>>) {
  return {
    ...
    ...(a.installYear !== undefined && { install_year: a.installYear }),
    ...(a.expectedLifespan !== undefined && { expected_lifespan: a.expectedLifespan }),
    ...(a.warrantyExpiry !== undefined && { warranty_expiry: a.warrantyExpiry || null }),
    ...(a.lastServiceDate !== undefined && { last_service_date: a.lastServiceDate || null }),
